...
data = load('murhythmdataset.mat'); 

T = table('RowNames', subject_ids);
T.data = data;
...
%     ax = subplot(10,5,subplot_index);
    [peak_frequency, peak_SNR] = estimate_SNR(epochs, T(row_index,:).fs, PEAK_FREQUENCY_INTERVAL, []);
...
% EDIT on 4/6/2020
% Ani: I don't want the plots so above I have put ax for estimate_SNR to
% [] and below commented out subplot editing.
    % beautify axes
%     legend off
...
for row_index = 1:height(T)
    subject = T(row_index,:).Row{1};
    % have created my own version of create_epochs_overlapping (below) which
    % doesn't create overlapping epochs. Need to email Zrenner about this.
    epochs = create_epochs_overlapping(T(row_index,:).data, T(row_index,:).fs); % from continuous data
...
%     
% EDIT: 4/6/2020
% Ani: I am removing the IIR filters as the correction for phase distortion
% is always imperfect. However, FIR filters should be equivalent as
% suggested by sameni 2017
% Changed it back to get same estimates for phase
% as they did in their paper % 4/24/2020
